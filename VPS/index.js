  ...
  app.post("/", async (req, res) => {
    ...
    try {
      switch (action) {
        ...
        case "EnviarDigitando": {
          const telefone = message?.telefone;
          if (!telefone) return res.status(400).json({ error: "❗ Telefone obrigatório" });
          const out = await funcoes.enviarDigitando(instancias, usuario, telefone);
          return res.json(toSafeJSON(out));
        }
        ...
        case "PararDigitando": {
          const telefone = message?.telefone;
          if (!telefone) return res.status(400).json({ error: "❗ Telefone obrigatório" });
          const out = await funcoes.pararDigitando(instancias, usuario, telefone);
          return res.json(toSafeJSON(out));
        }
        ...
        case "EnviarGravando": {
          const telefone = message?.telefone;
          if (!telefone) return res.status(400).json({ error: "❗ Telefone obrigatório" });
          const out = await funcoes.enviarGravando(instancias, usuario, telefone);
          return res.json(toSafeJSON(out));
        }
        ...
        case "GetGroupParticipants": {
          const idGrupo = message?.idGrupo;
          if (!idGrupo) return res.status(400).json({ error: "❗ idGrupo obrigatório" });
          const out = await funcoes.extrairParticipantesGrupo(instancias, usuario, idGrupo);
          return res.json(toSafeJSON(out));
        }
        ...
        case "GetProfilePic": {
          ...
          try {
            ...
            const url = await instancia.socket.profilePictureUrl(jid, 'image');
